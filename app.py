...
index_page = html.Div(style = {'font-family': 'Helvetica', 'background-color': '#ffffff', 'overflow': 'hidden', 'background-color': '#275968'}, children =[
    html.Div(style = {'font-family': 'Helvetica', 'font-size': '15','max-width': '100%','margin': 'auto', 'padding-left': '25px', 'padding-top': '20px','background-color': '#275968'}, children=[
        html.H1(style = {'font-family': 'Helvetica', 'font-size': '33','color': '#ffffff'},children='Escoja qué tipo de información desea ver:')
    ]),
...
baseMes = pd.read_csv('https://raw.githubusercontent.com/nicolasescobar0325/dash1/master/INFORME_DV_MENSUAL.csv', delimiter = ',')
...
page_1_layout = html.Div(style = {'font-family': 'Helvetica', 'height': '100%', 'background-color': '#ffffff', 'margin': '-8px', 'overflow': 'hidden'}, children =[
    html.Div(style = {'width': '30%', 'height': '100vh', 'background-color': '#275968', 'margin': '0px', 'float': 'left', 'box-shadow': '4px 5px 9px -3px rgba(102,102,102,1)'}, children =[
        ...
        html.Div(style = {'padding-left': '15px', 'padding-right': '15px', 'padding-top': '20px', 'font-size': '13'}, children = [
            ...
            html.Br(),
            html.P(style = {'color': '#ffffff', 'font-family': 'Helvetica', 'font-size': '16'}, children = 'Seleccione localidad:'),
            dcc.Dropdown(
                id ='loc-drop',
                clearable = False
                ),
    ...
    html.Div(style = {'width': '70%', 'height': '100%', 'margin': '0px', 'overflow': 'hidden', 'padding-top': '30px', 'padding-bottom': '5px'}, children =[
        html.Div(style = {'width': '100%', 'height': '320px', 'margin': '0px', 'overflow': 'hidden', 'padding-top': '30px'}, children = [
            ...
            html.Div(style = {'width': '50%', 'height': '100%', 'margin': '0px', 'float': 'left'}, children = [
                html.P(style = {'color': '#7f7f7f', 'font-family': 'Helvetica', 'font-size': '17', 'text-align': 'center'}, children = 'Cumplimiento Día (%)'),
...
def set_cities_options(selected_reg):
    filtro = base[base['PRODUCTO'] == selected_reg]
    return [{'label': i, 'value': i} for i in filtro['LOCALIDAD'].unique()]

@app.callback(
    dash.dependencies.Output('loc-drop', 'value'),
    [dash.dependencies.Input('sgro-drop', 'value')])
def set_cities_value(selected_reg):
...
def set_cities_options(selected_sgro, selected_loc):
    filtro = base[(base['PRODUCTO'] == selected_sgro)&(base['LOCALIDAD'] == selected_loc)]
    return [{'label': i, 'value': i} for i in filtro['DIRECTOR_VENTAS'].unique()]

@app.callback(
    dash.dependencies.Output('dir-drop', 'value'),
    [dash.dependencies.Input('loc-drop', 'options')])
def set_cities_value(available_options_sgro):
    return available_options_sgro[0]['value']
...
baseMes2 = pd.read_csv('https://raw.githubusercontent.com/nicolasescobar0325/dash1/master/INFORME_INFORMADOR_MENSUAL.csv', delimiter = ',')
...
page_2_layout = html.Div(style = {'font-family': 'Helvetica', 'height': '100%', 'background-color': '#ffffff', 'margin': '-8px', 'overflow': 'hidden'}, children =[
    ...
    html.Div(style = {'width': '70%', 'height': '100%', 'margin': '0px', 'overflow': 'hidden', 'padding-top': '30px', 'padding-bottom': '5px'}, children =[
        html.Div(style = {'width': '100%', 'height': '320px', 'margin': '0px', 'overflow': 'hidden', 'padding-top': '30px'}, children = [
            ...
            html.Div(style = {'width': '50%', 'height': '100%', 'margin': '0px', 'float': 'left'}, children = [
                html.P(style = {'color': '#7f7f7f', 'font-family': 'Helvetica', 'font-size': '17', 'text-align': 'center'}, children = 'Recaudo primas Día ($)'),
...
def update_figure(selected_dir2, selected_inf):
    ...
    y = np.round(base_filtrob2['RECAUDO_ACUMULADO'],1),
    text = 'Informador',
    name = 'Informador',
    marker = {'color': '#3381ff'},
    mode = "lines",
    ...
    trace3 = Scatter(
    x = base_filtrob2['FECHA'],
    y = np.round(base_filtrob1['RECAUDO_ACUMULADO'][base_filtrob1['NOMBRE_INFORMADOR_VENTAS']=='(Todos)'],1),
    text = 'Total',
    name = 'Total',
    marker = {'color': '#23c052'},
    mode = "lines",
...
def update_figure(selected_dir2, selected_inf):
    ...
    trace1 = Scatter(
    x = base_filtrob2['FECHA'],
    y = np.round(base_filtrob2['RECAUDO_DIA'],1),
    text = 'Informador',
    name = 'Informador',
    marker = {'color': '#3381ff'},
    mode = "lines",
    ...
    trace3 = Scatter(
    x = base_filtrob2['FECHA'],
    y = np.round(base_filtrob1['RECAUDO_DIA'][base_filtrob1['NOMBRE_INFORMADOR_VENTAS']=='(Todos)'],1),
    text = 'Total',
    name = 'Total',
    marker = {'color': '#23c052'},
    mode = "lines",
...
baseMes3 = pd.read_csv('https://raw.githubusercontent.com/nicolasescobar0325/dash1/master/INFORME_ASESOR_MENSUAL.csv', delimiter = ',')
...
page_4_layout = html.Div(style = {'font-family': 'Helvetica','height': '100%','background-color': '#ffffff', 'margin': '-8px', 'overflow': 'hidden'}, children =[
    html.Div(style = {'width': '20%', 'height':'1000px', 'color': '#ffffff', 'background-color': '#275968', 'margin': '0px', 'float': 'left', 'box-shadow': '4px 5px 9px -3px rgba(102,102,102,1)'}, children =[
        ...
        html.A(html.Button('Ir a Dashboard Localidad+Director', className='btn btn-primary',style = {'white-space': 'initial','overflow': 'visible','font-family': 'Helvetica', 'font-size': '14', 'width': '80%', 'background-color': '#068D8D', 'border-color':'#A8CECE','margin': 'auto', 'display':'block', 'box-shadow': '2px 2px 1px #A8CECE'}),
            href='/page-1'),
        html.A(html.Button('Ir a Dashboard Director+Informador', className='btn btn-primary btn-block',style = {'white-space': 'initial','overflow': 'visible','font-family': 'Helvetica', 'font-size': '14', 'width': '80%', 'background-color': '#068D8D', 'border-color':'#A8CECE','margin': 'auto', 'display':'block', 'box-shadow': '2px 2px 1px #A8CECE'}),
            href='/page-2'),
        html.A(html.Button('Ir a Dashboard Director+Asesor', className='btn btn-primary btn-block',style = {'white-space': 'initial','overflow': 'visible','font-family': 'Helvetica', 'font-size': '14', 'width': '80%', 'background-color': '#068D8D', 'border-color':'#A8CECE','margin': 'auto', 'display':'block', 'box-shadow': '2px 2px 1px #A8CECE'}),
            href='/page-3'),
        html.Br(),
        html.A(html.Button('Ir a Ranking Director', className='btn btn-primary btn-block',style = {'white-space': 'initial','overflow': 'visible','font-family': 'Helvetica', 'font-size': '14', 'width': '80%', 'background-color': '#068D8D', 'border-color':'#A8CECE','margin': 'auto', 'display':'block', 'box-shadow': '2px 2px 1px #A8CECE'}),
            href='/page-5'),
        html.Br(),
        html.A(html.Button('Ir al Inicio', className='btn btn-primary btn-block',style = {'white-space': 'initial','overflow': 'visible','font-family': 'Helvetica', 'font-size': '14', 'width': '80%', 'background-color': '#068D8D', 'border-color':'#A8CECE','margin': 'auto', 'display':'block', 'box-shadow': '2px 2px 1px #A8CECE'}),
            href='/')
    ]),
    html.Div(style = {'width': '80%', 'height': '100%', 'margin': '0px', 'overflow': 'hidden', 'padding-top': '35px', 'padding-bottom': '5px', 'font-family': 'Helvetica', 'font-size': '13'}, children =[
        html.P(style = {'color': '#7f7f7f', 'font-family': 'Helvetica', 'font-size': '17', 'text-align': 'center'}, children = 'Top 10 Mejores Localidades por %Cumplimiento'),
